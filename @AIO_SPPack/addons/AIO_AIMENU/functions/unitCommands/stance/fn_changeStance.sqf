...
if (_index == -1) then {
	...
	sleep 1;
	{
		...
		if (_index == 1 || _index == 4 || _index ==7) then {AIO_chosenUnitStance = ""} else {AIO_chosenUnitStance = _stance};
		_EH = _x addEventHandler ["AnimChanged", 
		{
			...
			if (_anim isEqualTo AIO_chosenUnitStance) then {
				[_unit] spawn {
					...
					sleep 2;
					_var = _unit getVariable ["AIO_AnimStateChanging1", 0];
					if (_var == 0) then {
						_unit switchMove AIO_chosenUnitStance;
						_unit setUnitPos AIO_posSelectIndex;
						_EH = _unit getVariable "AIO_StanceAnimChangedEH2";
