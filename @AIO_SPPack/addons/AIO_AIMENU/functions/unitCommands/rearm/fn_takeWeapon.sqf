...
while {!moveToCompleted _unit || _unit distance _tarPos > (_size/2.5 + 5)} do 
{
	if (!alive _AmmoBox || !alive _unit || currentCommand _unit != "STOP") exitWith {};
	sleep 1;
};
_unit doWatch objNull;
if (_unit distance _tarPos < (_size/2.5 + 5)) then {
