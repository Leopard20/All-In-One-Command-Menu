...
for "_i" from 0 to (_count - 1) do
{
	...
	if (vehicle _unit1 != _unit1) then {
		_crew = (fullCrew [(vehicle _unit1), "", false]) select {_x select 1 == "Gunner" || _x select 1 == "Driver"};
		...
		if (_unit1 != effectiveCommander (vehicle _unit1) || effectiveCommander (vehicle _unit1) == player) exitWith {_units = _units - [_unit1, (vehicle _unit1)]};
		_unit1 doMove _pos;
		[_unit1] spawn {
			params ["_unit1"];
...
_assign_Pos =
{
	...
	if (count _statics > 0) then {
		for "_j" from 0 to (count _statics - 1) do
		{
			...
			if ({_x select 0 == _static} count AIO_chosen_defense < 1) then {
						AIO_chosen_defense pushBack [_static, [0,0,0]]; _assigned = true;};
			if (_assigned) exitWith {[[_unit], _static, 0, false] call AIO_fnc_getIn};
		};
	};
			...
			if (count _positions > 0) then {
				for "_i" from 0 to (count _positions - 1) do
				{
					...
					if ({_x select 0 == _house && _x select 1 isEqualTo _position} count AIO_chosen_defense < 1) then {
						AIO_chosen_defense pushBack [_house, _position]; _cover = _position; breakOut "AIO_HousePos"};
	...
	[[_unit], 30, false] spawn AIO_fnc_takeCover;
};
_diff = 360/_count;
_movePos = [];
for "_i" from 0 to (_count - 1) do 
...
_remUnits = [];
{
	...
	{
		_remUnits pushBack _x;
	};
} forEach _units;
if (_radius >= 5) then {_radius = _radius/2 + 2};
[_remUnits, _pos, _radius] spawn AIO_fnc_defense360;