...
call {
	...
	if (isNull _target) then {_watchPos = _param1; _searchType = 1; _target = _unit} else {_watchPos = (ASLToAGL(getPosASL _target))};
	...
	_houses = nearestObjects [_pos, ["HOUSE"], 30];
	...
	if (count _goodCovers > 0) exitWith {
		...
		_goodCovers = [_goodCovers, [], {(_x select 0) distance2D _pos}, "ASENCD"] call BIS_fnc_sortBy;
		...
		_queue pushBack [3, _selectedPos, _bestCover, 2];
	};
	...
	if (count _covers > 0) exitWith {
		_covers = [_covers, [], {(_x select 0) distance _pos}, "ASCEND"] call BIS_fnc_sortBy;
		...
		if (count(_bestCover buildingPos -1) - ({alive (_x select 0)} count (_bestCover getVariable ["AIO_entrySync", []])) > 0) then {
			...
			_queue pushBack [3, _selectedPos, _bestCover, 1];
		} else {
	...
	_softCovers = ((nearestTerrainObjects [_pos, ["HIDE", "BUSH", "TREE", "ROCKS"], 30]) select {_obj = _x; _index = _takenCovers findIf {_x select 0 == _obj}; (([_x, 0.2, 1] call AIO_fnc_isBigEnough) && {_index == -1 || {{alive _x} count ((_takenCovers select _index) select 1) == 0}})}) + ((nearestObjects [_pos, ["HELICOPTER","PLANE"], 30]) select {_obj = _x; _index = _takenCovers findIf {_x select 0 == _obj}; ((isNull (driver _x)) && {_index == -1 || {{alive _x} count ((_takenCovers select _index) select 1) == 0}})});
	
	if (count _softCovers == 0) exitWith {_queue pushBack [3, _pos, objNull, 0]};
	
	_softCovers = [_softCovers, [], {_x distance _pos}, "ASCEND"] call BIS_fnc_sortBy;
	...
	_queue pushBack [3, _selectedPos, _bestCover, 0];
};
