params ["_Road", "_lastRoad", "_lastDir"];
_roadPos = (getPosASL _Road) vectorAdd [0,0,0.85];
...
private _redList = [_Road, _lastRoad];
private _radius = 20;
_connections = [];

while {count _connections == 0 && _radius <= 50} do {
	_connections = (_Road nearRoads _radius) select {!(_x in _redList) && {_x inArea AIO_searchArea &&
	{count ((lineIntersectsSurfaces [_roadPos, (getPosASL _x) vectorAdd [0,0,0.85], objNull, objNull, true, 2, "GEOM", "NONE"]) select {!((_x select 3) isKindOf "MAN")}) == 0}}}; 
	_radius = _radius + 10;
};
//if (AIO_Taxi_endRoad in _connections) exitWith {[[_Road]]};

_connections = _connections select {!(_x in AIO_checkedRoads)};
...
private _results = [];
AIO_checkedRoads pushBack _Road;
