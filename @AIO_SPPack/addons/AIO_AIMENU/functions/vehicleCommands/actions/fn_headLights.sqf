...
_disable_lights_fnc =
{
	...
	if (vehicle _unit == _unit || (_vehicle isKindOf "Ship")) exitWith {};
	_hitPoints = (getAllHitPointsDamage vehicle _unit) select 0;
	_hitNames = (getAllHitPointsDamage vehicle _unit) select 1;
	_lights = [];
	_invalid = (["Prowler", _name] call BIS_fnc_inString) || (["Bike", _name] call BIS_fnc_inString) || (["MSE-3", _name] call BIS_fnc_inString) || (["Qilin", _name] call BIS_fnc_inString);
	for "_i" from 0 to ((count _hitNames) - 1) do
	{
		_isLight = if (_author == "Bohemia Interactive" && !(_vehicle isKindOf "Air") && !_invalid) then {(["light", (_hitNames select _i)] call BIS_fnc_inString) || (["light", (_hitPoints select _i)] call BIS_fnc_inString) || (_hitNames select _i == "")} else {(["light", (_hitNames select _i)] call BIS_fnc_inString) || (["light", (_hitPoints select _i)] call BIS_fnc_inString)};
		if (_isLight) then {
			_lights pushBack _i;
		};
	};
